...
from scripts.particule import Particule
...
class Game:
    ...
    def run(self):
        ...
        while True:
            ...
            for rect in self.leaf_spawners:
                if random.random() * 49999 < rect.width * rect.height:
                    ...
                    self.particules.append(
                        Particule(self, "leaf", pos, velocity=[-0.1, 0.3], frame=random.randint(0, 20))
                    )
            ...
            for particule in self.particules.copy():
                kill = particule.update()
                particule.render(self.display, offset=render_scroll)
                if particule.type == "leaf":
                    particule.pos[0] += math.sin(particule.animation.frame * 0.035) * 0.3
                if kill:
